/**
 * Mia Software Development (mia-software-dev) — runnable JS build.
 * =========================================================================
 * Board task 2E. Built to the owner (Luis) requirements (2026-06-22 call): build
 * software FOR a non-technical user, secure-by-default, on **SQLite**, with a
 * front end + back end, asking about auth before exposing data, and escalating to
 * Voltek when stuck. The RULES live in the bundled skill (skills/.../SKILL.md);
 * these tools create the structure the skill works inside.
 *
 * Tools:
 *   scaffold_project — create projects/<slug>/ (structured-work files +
 *                      frontend/ + backend/ skeleton on SQLite, base tables).
 *   add_table        — append a table to a project's backend/schema.sql.
 *
 * Storage is the agent workspace; no core changes. index.ts is the typed
...
function scaffoldFiles(slug, name, desc, authChoice, isPublic) {
  const authLine =
    authChoice === "yes"
      ? "Required (login)"
      : authChoice === "no"
...
function doScaffold(args, cfg) {
  ...
  const authChoice = ["yes", "no", "ask"].includes(String(args.auth)) ? String(args.auth) : "ask";
    ...
    api.registerTool(
      {
        ...
        execute: async (_toolCallId, rawParams) => doScaffold(rawParams && typeof rawParams === "object" ? rawParams : {}, cfg),
      },
      { name: "scaffold_project" },
    );
    ...
    api.registerTool(
      {
        ...
        execute: async (_toolCallId, rawParams) => doAddTable(rawParams && typeof rawParams === "object" ? rawParams : {}, cfg),
      },
      { name: "add_table" },
    );
  },
